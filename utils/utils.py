...
def metrics(y_true, y_pred, category, category_dict):
    ...
    for c, res in res_by_category.items():
        try:
            if len(res['y_true']) > 0 and len(res['y_pred']) > 0:
                metrics_by_category[c] = {
    ...
    for c, res in res_by_category.items():
        ...
        try:
            # 确保 y_pred 是列表或数组，并转换为 numpy 数组
            if len(res['y_true']) == 0 or len(res['y_pred']) == 0:
                ...
                # 确保转换为 numpy 数组
                y_pred_arr = np.asarray(res['y_pred'])
                y_pred_int = np.around(y_pred_arr).astype(int)
                
                metrics_by_category[c] = {
                    'precision': round(precision_score(res['y_true'], y_pred_int, average='macro', zero_division=0), 4),
                    'recall': round(recall_score(res['y_true'], y_pred_int, average='macro', zero_division=0), 4),
                    'fscore': round(f1_score(res['y_true'], y_pred_int, average='macro', zero_division=0), 4),
